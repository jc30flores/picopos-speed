...
const actionConfig = (row: DTERecord) => [
  ...
  { key: "email" as const, label: "Enviar correo", enabled: Boolean(row.can_send_email), reason: row.missing_email_reason || "" },
    ...
    <TooltipProvider delayDuration={120}>
      <div className="flex items-center justify-end gap-1">
        {actionConfig(row).map((action) => {
          ...
          const tip = action.enabled || action.key === "view" ? action.label : (action.reason || `${action.label} no disponible`);
          return (
