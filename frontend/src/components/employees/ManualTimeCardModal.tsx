...
export function ManualTimeCardModal({ open, onClose, employees, onSaved, initialEmployeeId, lockEmployeeSelection=false }: { open: boolean; onClose: () => void; employees: EmployeeOpt[]; onSaved: () => void; initialEmployeeId?: number | null; lockEmployeeSelection?: boolean; }) {
  ...
  const submit = async () => {
    const error = validateTimeCardPayload({ employeeId, date, clockInTime, breakStartTime, breakEndTime, clockOutTime });
  ...
  return <Dialog open={open} onOpenChange={(v) => !v && onClose()}><DialogContent className='max-w-2xl'><DialogHeader><DialogTitle>Nueva Tarjeta de Horas</DialogTitle></DialogHeader>
    ...
    <div className='grid gap-3 md:grid-cols-2'>
      <div><Label>Empleado</Label><Select value={employeeId ? String(employeeId) : ''} onValueChange={(v) => setEmployeeId(Number(v))} disabled={lockEmployeeSelection}><SelectTrigger><SelectValue placeholder='Selecciona empleado' /></SelectTrigger><SelectContent>{filtered.map((e)=><SelectItem key={e.id} value={String(e.id)}>{e.name} — {formatRole(e.role)}</SelectItem>)}</SelectContent></Select></div>
