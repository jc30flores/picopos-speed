...
const parseLocalDate = (value: string) => { const [y, m, d] = value.split("-").map(Number); return new Date(y, (m || 1) - 1, d || 1); };
...
const formatDuration = (m: number | null | undefined) => { const mm = Math.max(0, Math.round(Number(m || 0))); return `${Math.floor(mm / 60)}h ${String(mm % 60).padStart(2, "0")}m`; };
...
const fmtTime = (value?: string | null) => value ? new Date(value).toLocaleTimeString("es-SV", { hour: "2-digit", minute: "2-digit", hour12: false }) : "—";
...
export const EmployeeHoursTab = () => {
  ...
  const [loading, setLoading] = useState(false); const [open, setOpen] = useState(false); const [editOpen, setEditOpen] = useState(false); const [edit, setEdit] = useState<EditState | null>(null);
  ...
  const dayRows = useMemo(() => {
    ...
    const list: any[] = [];
    for (let day = parseLocalDate(dateFrom); formatLocalDate(day) <= dateTo; day = new Date(day.getFullYear(), day.getMonth(), day.getDate() + 1)) {
  ...
  return <div className="space-y-4">
    ...
    <Card><CardContent className="pt-6"><Table><TableHeader><TableRow><TableHead>Empleado</TableHead><TableHead>Rol</TableHead><TableHead>Días</TableHead><TableHead>Entradas</TableHead><TableHead>Salidas</TableHead><TableHead>Horas turno</TableHead><TableHead>Breaks</TableHead><TableHead>Horas netas</TableHead></TableRow></TableHeader><TableBody>{loading ? <TableRow><TableCell colSpan={8}>Cargando...</TableCell></TableRow> : rows.map((r) => <TableRow key={r.employeeId} className="cursor-pointer" onClick={() => void detail(r.employeeId)}><TableCell>{r.name}</TableCell><TableCell>{r.role}</TableCell><TableCell>{r.daysWorked}</TableCell><TableCell>{r.entriesCount}</TableCell><TableCell>{r.exitsCount}</TableCell><TableCell>{formatDuration(r.shiftMinutes)}</TableCell><TableCell>{formatDuration(r.breakMinutes)}</TableCell><TableCell>{formatDuration(r.netMinutes)}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card>

    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-6xl"><DialogHeader><DialogTitle>Tarjeta de Horas — {selected?.employee.name}</DialogTitle></DialogHeader>{isAdmin ? <p className="text-xs text-muted-foreground">Toca una fila para editar o crear un registro.</p> : null}<div className="max-h-[420px] overflow-auto rounded-xl border"><table className="w-full"><thead><tr><th>FECHA</th><th>ENTRADA</th><th>SALIDA</th><th>TOTAL DE HORAS TRABAJADAS</th><th>TOTAL DE DESCANSO</th><th>TOTAL FINAL</th></tr></thead><tbody>{dayRows.map((c, i) => <tr key={i} className={`${isAdmin ? "cursor-pointer hover:bg-muted/40" : ""}`} onClick={() => openEdit(c)}><td>{c.date}</td><td>{c.isEmpty ? "—" : fmtTime(c.clockInAt)}</td><td>{c.isEmpty ? "—" : (c.clockInAt && !c.clockOutAt ? "En curso" : fmtTime(c.clockOutAt))}</td><td className="text-emerald-700">{c.isEmpty ? "—" : formatDuration(c.shiftMinutes)}</td><td className="text-amber-700">{c.isEmpty ? "—" : formatDuration(c.breakMinutes)}</td><td className="text-sky-700">{c.isEmpty ? "—" : formatDuration(c.netMinutes)}</td></tr>)}</tbody></table></div></DialogContent></Dialog>
