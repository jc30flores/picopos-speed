...
export const EmployeeHoursTab = () => {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid grid-cols-1 gap-3 md:grid-cols-4">{summaryCards.map((item) => <Card key={item.label}><CardContent className="p-4"><p className="text-xs text-muted-foreground">{item.label}</p><p className={`text-xl font-bold ${item.label.includes("turno") ? "text-emerald-700 dark:text-emerald-300" : item.label.includes("break") ? "text-amber-700 dark:text-amber-300" : item.label.includes("netas") ? "text-sky-700 dark:text-sky-300" : ""}`}>{item.value}</p></CardContent></Card>)}</div>
