...
export const EmployeeHoursTab = () => {
  ...
  return <div className="space-y-4">
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-6xl"><DialogHeader><DialogTitle>Tarjeta de Horas — {selected?.employee.name}</DialogTitle></DialogHeader>{isAdmin ? <p className="text-xs text-muted-foreground">Toca una fila para editar o crear un registro.</p> : null}<div className="max-h-[420px] overflow-auto rounded-xl border"><table className="w-full"><thead><tr><th>FECHA</th><th>ENTRADA</th><th>SALIDA</th><th>TOTAL DE HORAS TRABAJADAS</th><th>TOTAL DE DESCANSO</th><th>TOTAL FINAL</th></tr></thead><tbody>{dayRows.length === 0 ? <tr><td colSpan={6} className="p-3 text-center text-muted-foreground">Sin registros</td></tr> : dayRows.map((c, i) => <tr key={i} className={`${isAdmin ? "cursor-pointer hover:bg-muted/40" : ""}`} onClick={() => openEdit(c)}><td>{c.date}</td><td>{c.isEmpty ? "—" : fmtTime(c.clockInAt)}</td><td>{c.isEmpty ? "—" : (c.clockInAt && !c.clockOutAt ? "En curso" : `${fmtTime(c.clockOutAt)}${c.clockOutNextDay ? " (+1)" : ""}`)}</td><td className="text-emerald-700">{c.isEmpty ? "—" : formatDuration(c.shiftMinutes)}</td><td className="text-amber-700">{c.isEmpty ? "—" : formatDuration(c.breakMinutes)}</td><td className="text-sky-700">{c.isEmpty ? "—" : formatDuration(c.netMinutes)}</td></tr>)}</tbody></table></div></DialogContent></Dialog>
