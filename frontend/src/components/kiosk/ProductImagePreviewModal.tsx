    ...
    >
      <div className="w-full max-w-4xl overflow-hidden rounded-2xl border border-white/10 bg-slate-950 text-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
          ...
          <button
            type="button"
            className="rounded-lg p-2 hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-emerald-500"
            onClick={onClose}
            aria-label="Cerrar"
          >
            <X className="h-5 w-5" />
        ...
        <div className="grid grid-cols-1 gap-0 md:grid-cols-5">
          <div className="flex items-center justify-center bg-black/30 p-4 md:col-span-3">
            {imageSrc ? (
              <img
                src={imageSrc}
                alt={item.name}
              ...
              <div className="flex h-[320px] w-full items-center justify-center rounded-xl bg-white/5 text-white/60">
                Sin imagen
              </div>
            )}
          </div>
          ...
          <div className="p-5 md:col-span-2">
            ...
            {item.description ? (
              ...
              <p className="mt-3 text-base text-white/50">Sin descripción.</p>
            )}
            <div className="mt-6 flex items-center justify-between">
              <div className="text-white/70">Precio</div>
