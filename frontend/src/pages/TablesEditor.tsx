...
const initialTableForm: TableForm = { area: "", name: "", number: "1", capacity: "4", shape: "round", width: "120", height: "80", rotation: "0", color: "#1F7A4D", isActive: true, autoName: true };
...
export default function TablesEditor() {
  ...
  useEffect(() => {
    getRuntimeFeatureSettings().then((s) => setEnabled(Boolean(s.tableMapEnabled && s.operationMode !== "quick_pos"))).catch(() => setEnabled(false));
  }, []);
  ...
  if (!enabled) return <div className="p-6"><Card className="p-6">Modo mesas no está activo. Cambia el modo de operación desde Configuración &gt; Funciones.</Card></div>;
      ...
      <div className="flex min-h-0 min-w-0 flex-col gap-2 overflow-hidden">
        <Card className="p-2">
          <div className="flex flex-wrap items-center gap-1">
            <Button size="icon" variant="outline" className="h-8 w-8 gp-primary-border gp-primary-text" title="Volver al menú principal" aria-label="Volver al menú principal" onClick={() => navigate("/")}><ArrowLeft className="h-4 w-4" /></Button>
