...
export default function TablesEditor() {
  ...
  const patchTable = (id: number, patch: Partial<RestaurantTable>) => {
    setTables((prev) => {
      ...
      pendingLayoutRef.current[id] = { x: next.x, y: next.y, width: next.width, height: next.height, rotation: next.rotation };
      return prev.map((t) => (t.id === id ? next : t));
    });
  };

  const getNextNameAndNumber = (base: RestaurantTable) => {
    const maxNumber = Math.max(0, ...tables.map((t) => Number(t.number || 0)));
    const nextNumber = maxNumber + 1;
    const match = /^(.*?)(\d+)$/.exec(base.name.trim());
    const nextName = match ? `${match[1].trim()} ${nextNumber}` : `${base.name} copy ${tables.filter((t) => t.name.startsWith(base.name)).length + 1}`;
    return { nextNumber, nextName };
  };

  const duplicateTable = async (base: RestaurantTable) => {
    const { nextName, nextNumber } = getNextNameAndNumber(base);
    const created = await createRestaurantTable({ ...base, id: undefined as never, name: nextName, number: nextNumber, x: base.x + 32, y: base.y + 32 });
  ...
  useEffect(() => {
    const onMove = (e: PointerEvent) => {
      ...
      if (op.type === "drag") {
        ...
        patchTable(op.id, { x: snapOn ? snap(nx) : nx, y: snapOn ? snap(ny) : ny });
        return;
      }
      if (op.type === "resize") {
        ...
        if (snapOn) { x = snap(x); y = snap(y); width = snap(width); height = snap(height); }
        patchTable(op.id, { x: Math.max(0, x), y: Math.max(0, y), width: Math.max(minW, width), height: Math.max(minH, height) });
        return;
      }
      if (op.type === "rotate") {
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      ...
      if (meta && e.key.toLowerCase() === "v" && clipboardRef.current) {
        e.preventDefault();
        void duplicateTable({ ...clipboardRef.current, id: selected?.id ?? -1 } as RestaurantTable);
      }
  ...
  return <div className="h-[100dvh] overflow-hidden bg-background p-2 md:p-3">
    <div className="grid h-full min-h-0 grid-cols-1 gap-2 lg:grid-cols-[210px_1fr]">
      ...
      <div className="flex min-h-0 flex-col gap-2">
        <Card className="p-2">
          <div className="flex flex-wrap items-center gap-1">
            <Input className="h-8 w-44" placeholder="Buscar mesa" value={search} onChange={(e) => setSearch(e.target.value)} />
            <Button size="sm" variant="outline" onClick={() => setTableModal({ open: true })}>+ Mesa</Button>
            ...
            <Button size="sm" variant="outline" onClick={() => setZoom((z) => Math.max(0.5, z - 0.1))}>-</Button><span className="text-xs">{Math.round(zoom * 100)}%</span><Button size="sm" variant="outline" onClick={() => setZoom((z) => Math.min(2, z + 0.1))}>+</Button>
            <Label className="ml-1 flex items-center gap-1 text-xs" title="Ajustar a cuadrícula"><Switch checked={snapOn} onCheckedChange={setSnapOn} />Snap {snapOn ? 'ON' : 'OFF'}</Label>
        ...
        <Card className="min-h-0 flex-1 p-2">
          <div className="h-full min-h-0 overflow-auto rounded border bg-slate-950" onPointerDown={(e) => { if (e.target === e.currentTarget) { setSelectedTableId(null); setTableCtx({ open: false, x: 0, y: 0, id: null }); } }}>
