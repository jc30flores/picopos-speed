...
const palette = ["#10b981", "#3b82f6", "#f97316", "#8b5cf6", "#ef4444", "#6b7280"];
const GRID = 10;
const snap = (n:number)=> Math.round(n/GRID)*GRID;
const clamp = (n:number,min:number,max:number)=> Math.max(min, Math.min(max,n));
function getRecommendedTableSize(shape: RestaurantTable["shape"], capacity: number) {
  if (shape === "round") return capacity <= 2 ? { width: 90, height: 90 } : capacity <= 4 ? { width: 110, height: 110 } : capacity <= 6 ? { width: 130, height: 130 } : { width: 150, height: 150 };
  if (shape === "square") return capacity <= 2 ? { width: 90, height: 90 } : capacity <= 4 ? { width: 115, height: 115 } : capacity <= 6 ? { width: 135, height: 135 } : { width: 155, height: 155 };
  if (shape === "rectangle") return capacity <= 4 ? { width: 150, height: 90 } : capacity <= 6 ? { width: 180, height: 100 } : capacity <= 8 ? { width: 220, height: 110 } : { width: 260, height: 120 };
  if (shape === "booth") return capacity <= 2 ? { width: 130, height: 80 } : capacity <= 4 ? { width: 170, height: 90 } : capacity <= 6 ? { width: 210, height: 100 } : { width: 240, height: 110 };
  return capacity <= 4 ? { width: 180, height: 70 } : capacity <= 6 ? { width: 240, height: 75 } : capacity <= 8 ? { width: 300, height: 80 } : { width: 360, height: 85 };
}

const TablesEditor = () => {
const navigate = useNavigate(); const canvasRef = useRef<HTMLDivElement | null>(null);
const [enabled, setEnabled] = useState<boolean | null>(null); const [areas, setAreas] = useState<DiningArea[]>([]); const [tables, setTables] = useState<RestaurantTable[]>([]);
const [selectedAreaId, setSelectedAreaId] = useState<number | null>(null); const [selectedTableId, setSelectedTableId] = useState<number | null>(null); const [query, setQuery] = useState("");
const [zoom, setZoom] = useState(1); const [snapToGrid, setSnapToGrid] = useState(true);
const [pendingLayout, setPendingLayout] = useState<Record<number, Pick<RestaurantTable, "x"|"y"|"width"|"height"|"rotation">>>({});
const [dragging, setDragging] = useState<{ id:number; startX:number; startY:number; dx:number; dy:number; moved:boolean }|null>(null);
const [resizing, setResizing] = useState<{ id:number; dir:string; sx:number; sy:number; start:RestaurantTable }|null>(null);
const [rotating, setRotating] = useState<{ id:number; cx:number; cy:number }|null>(null);
const [areaModal, setAreaModal] = useState<{ open:boolean; area?:DiningArea }>({open:false}); const [tableModal, setTableModal] = useState<{ open:boolean; table?:RestaurantTable }>({open:false});
...
useEffect(()=>{ if(enabled) reload().catch(()=>toast.error("No se pudo cargar el mapa.")); },[enabled]);
...
const filteredTables = useMemo(()=>{ const q=query.trim().toLowerCase(); return tables.filter(t=>(!selectedAreaId||t.area===selectedAreaId)&&(!q||t.name.toLowerCase().includes(q)||String(t.number).includes(q)));},[tables,selectedAreaId,query]);
...
const startDrag=(table:RestaurantTable,ev:React.PointerEvent<HTMLButtonElement>)=>{ (ev.currentTarget as HTMLElement).setPointerCapture(ev.pointerId); setSelectedTableId(table.id); setDragging({id:table.id,startX:ev.clientX,startY:ev.clientY,dx:ev.clientX-table.x*zoom,dy:ev.clientY-table.y*zoom,moved:false}); };
const startResize=(ev:React.PointerEvent, table:RestaurantTable, dir:string)=>{ ev.stopPropagation(); (ev.currentTarget as HTMLElement).setPointerCapture(ev.pointerId); setResizing({id:table.id,dir,sx:ev.clientX,sy:ev.clientY,start:{...table}}); };
const startRotate=(ev:React.PointerEvent, table:RestaurantTable)=>{ ev.stopPropagation(); const cx=table.x+table.width/2; const cy=table.y+table.height/2; setRotating({id:table.id,cx,cy}); };
const onMove=(ev:React.PointerEvent<HTMLDivElement>)=>{ if(dragging){ const nx=(ev.clientX-dragging.dx)/zoom; const ny=(ev.clientY-dragging.dy)/zoom; const x=snapToGrid?snap(nx):nx; const y=snapToGrid?snap(ny):ny; applyLocalTable(dragging.id,{x:Math.max(0,x),y:Math.max(0,y)}); if(Math.abs(ev.clientX-dragging.startX)>=4||Math.abs(ev.clientY-dragging.startY)>=4) setDragging({...dragging,moved:true}); }
if(resizing){ const dx=(ev.clientX-resizing.sx)/zoom; const dy=(ev.clientY-resizing.sy)/zoom; let {x,y,width,height}=resizing.start; const d=resizing.dir; if(d.includes('e')) width=clamp(width+dx,60,500); if(d.includes('s')) height=clamp(height+dy,50,500); if(d.includes('w')){ width=clamp(width-dx,60,500); x=resizing.start.x+(resizing.start.width-width);} if(d.includes('n')){ height=clamp(height-dy,50,500); y=resizing.start.y+(resizing.start.height-height);} if(ev.shiftKey&&d.length===2){ const z=Math.max(width,height); width=z;height=z; } applyLocalTable(resizing.id,{x,y,width,height}); }
if(rotating){ const t=tables.find(x=>x.id===rotating.id); if(!t) return; const px=(ev.clientX/zoom); const py=(ev.clientY/zoom); let deg=((Math.atan2(py-rotating.cy,px-rotating.cx)*180/Math.PI)+90+360)%360; if(ev.shiftKey) deg=Math.round(deg/15)*15; applyLocalTable(rotating.id,{rotation:Number(deg.toFixed(1))}); }
};
...
{['nw','n','ne','e','se','s','sw','w'].map(h=><div key={h} onPointerDown={(e)=>startResize(e,t,h)} className="absolute w-3 h-3 bg-white border border-emerald-500 rounded-sm" style={{left:h.includes('w')?-6:h.includes('e')?t.width-6:t.width/2-6, top:h.includes('n')?-6:h.includes('s')?t.height-6:t.height/2-6}} />)}
</>:null}
</button>})}
</div></div>}</Card>
<Card className="p-4 space-y-3"><h2 className="font-semibold">Propiedades</h2>{!selectedTable?<p className="text-sm text-muted-foreground">Selecciona una mesa para editar rápidamente.</p>:<>{(()=>{const t=selectedTable; return <div className="space-y-2"><Label>Nombre</Label><Input value={t.name} onChange={(e)=>setTables(prev=>prev.map(x=>x.id===t.id?{...x,name:e.target.value}:x))}/><Label>Número</Label><Input value={String(t.number)} onChange={(e)=>setTables(prev=>prev.map(x=>x.id===t.id?{...x,number:Number(e.target.value||1)}:x))}/><Label>Capacidad</Label><Input value={String(t.capacity)} onChange={(e)=>setTables(prev=>prev.map(x=>x.id===t.id?{...x,capacity:Number(e.target.value||1)}:x))}/><Label>Color</Label><div className="flex gap-2 flex-wrap">{palette.map(c=><button key={c} className="w-6 h-6 rounded border" style={{background:c}} onClick={()=>setTables(prev=>prev.map(x=>x.id===t.id?{...x,color:c}:x))}/>)}</div><Input type="color" value={t.color||palette[0]} onChange={(e)=>setTables(prev=>prev.map(x=>x.id===t.id?{...x,color:e.target.value}:x))}/><Label>X / Y</Label><div className="grid grid-cols-2 gap-2"><Input value={String(Math.round(t.x))} onChange={(e)=>applyLocalTable(t.id,{x:Number(e.target.value||0)})}/><Input value={String(Math.round(t.y))} onChange={(e)=>applyLocalTable(t.id,{y:Number(e.target.value||0)})}/></div><Label>Ancho / Alto</Label><div className="grid grid-cols-2 gap-2"><Input value={String(Math.round(t.width))} onChange={(e)=>applyLocalTable(t.id,{width:Math.max(60,Number(e.target.value||60))})}/><Input value={String(Math.round(t.height))} onChange={(e)=>applyLocalTable(t.id,{height:Math.max(50,Number(e.target.value||50))})}/></div><Label>Rotación</Label><Input value={String(Math.round(t.rotation))} onChange={(e)=>applyLocalTable(t.id,{rotation:Number(e.target.value||0)})}/><div className="flex gap-2"><Button size="sm" onClick={async()=>{ await updateRestaurantTable(t.id,{name:t.name,number:t.number,capacity:t.capacity,color:t.color,area:t.area,shape:t.shape,width:t.width,height:t.height,rotation:t.rotation,x:t.x,y:t.y,isActive:t.isActive}); toast.success('Mesa guardada correctamente.'); await reload(); }}>Guardar cambios</Button><Button size="sm" variant="outline" onClick={()=>void duplicateTable(t)}>Duplicar</Button></div><Button size="sm" variant="destructive" onClick={async()=>{ if(!confirm('¿Deseas eliminar esta mesa?')) return; const r=await deleteRestaurantTable(t.id); toast.success(r.detail||'Mesa actualizada.'); await reload(); setSelectedTableId(null); }}>Eliminar/Desactivar</Button></div>})()}</>}</Card>
</div>
