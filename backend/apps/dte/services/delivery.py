    ...
    for channel in selected_channels:
        if channel == "email":
            if not flags.get("can_send_email"):
                ...
                provider_message = _provider_message(attempt.provider_body)
                results[channel] = _channel_result(
                    ok=attempt.status == "SENT",
                    status_code=attempt.provider_status,
                    error=provider_message or provider_error or ("No se pudo enviar correo" if attempt.status != "SENT" else None),
                )
        if channel == "whatsapp":
            if not flags.get("can_send_whatsapp"):
                ...
                results[channel] = _channel_result(
                    ok=attempt.status == "SENT",
                    status_code=attempt.provider_status,
                    error=provider_message or provider_error or ("No se pudo enviar WhatsApp" if attempt.status != "SENT" else None),
                )
