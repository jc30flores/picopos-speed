...
class SystemPrinterServiceTests(SimpleTestCase):
    def test_is_printer_available_checks_lpstat_v(self):
        ...
        with patch.object(
            service,
            "run_command",
            return_value=CommandResult(ok=True, exit_code=0, stdout="device for TSP143-(STR_T-001): usb://Star", stderr="", elapsed_ms=5, command="lpstat -v"),
        ):
    ...
    def test_open_cash_drawer_uses_lp_raw_with_bytes(self):
        ...
        with patch.object(service, "is_printer_available", return_value=True), patch.object(service, "run_command") as run_command:
            run_command.return_value = CommandResult(ok=True, exit_code=0, stdout="ok", stderr="", elapsed_ms=5, command="lp -d TSP143-(STR_T-001) -o raw")
            opened, error = service.open_cash_drawer()
        self.assertTrue(opened)
        self.assertIsNone(error)
        args, kwargs = run_command.call_args
        self.assertEqual(args[0], ["lp", "-d", "TSP143-(STR_T-001)", "-o", "raw"])
    ...
    def test_print_with_pdf_fallback_printer_ok(self):
        ...
        with patch.object(service, "check_queue_exists", return_value=True), patch.object(
            service,
            "run_command",
            return_value=CommandResult(ok=True, exit_code=0, stdout="ok", stderr="", elapsed_ms=5, command="lp -d TSP143-(STR_T-001) /tmp/x"),
        ):
    ...
    def test_print_with_pdf_fallback_lp_error_generates_pdf(self):
        ...
        with patch.object(service, "check_queue_exists", return_value=True), patch.object(
            service,
            "run_command",
            return_value=CommandResult(ok=False, exit_code=1, stdout="", stderr="Unknown printer", elapsed_ms=5, command="lp -d TSP143-(STR_T-001) /tmp/x"),
        ):
