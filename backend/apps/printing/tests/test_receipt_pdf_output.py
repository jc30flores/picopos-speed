...
class ReceiptPdfOutputTests(SimpleTestCase):
    ...
    def test_sale_pdf_displays_hacienda_values_and_hides_none(self):
        ...
        self.assertIn("CODIGO GEN", text)
        self.assertNotIn("PROCESADO", text)
        self.assertNotIn("25/04/2026 17:00:57", text)
        self.assertNotIn("DATOS DTE", text)
        self.assertNotIn("ESTADO DTE", text)
        self.assertNotIn("FECHA DTE", text)
        self.assertNotIn("PROCESAMIENTO", text)
        self.assertNotIn("ATENDIDO POR", text)
