...
Incluye imágenes, puerto local, secretos placeholder, DB, `ALLOWED_HOSTS`, CORS/CSRF, rutas POSIX, Gunicorn, y variables existentes para desactivar DTE/impresión/cajón.
...
El stack no está listo para producción Windows; faltan separar worker DTE, monitor DTE dedicado, backup/restore, scripts Windows, `compose.windows.yaml`, acceso LAN, agente impresión Windows, instalador `.exe`, actualización/rollback, prueba DTE real controlada y revisar migraciones pendientes si persisten.

## 24. Estado DTE
Docker usa variables existentes `DTE_MONITOR_ENABLED=false`, `DTE_OUTBOX_WORKER_ENABLED=false`, `DTE_SENTINEL_ENABLED=false`, `DTE_MAX_RETRIES=0`, `DTE_BASE_URL=` y `DTE_API_TOKEN=`. No se modificó `AppConfig.ready()`; en Gunicorn no debería arrancar por `RUN_MAIN`, pero la separación formal queda pendiente.
